        ...
        for j, metric in enumerate(metrics):
            ...
            for epoch in epochs:
                ...
                if log_dict[epoch]['mode'][-1] == 'val':
                    # print('epoch: ',epoch,'val  -  ','iter: ',iters,'top1: ',np.array(log_dict[epoch][metric][-1]))
                    ysVal.append(np.array(log_dict[epoch][metric][-1]))
                    xsVal.append(np.array(num_iters_per_epoch) + (epoch - 1) * num_iters_per_epoch)
                    iters = iters[:-1]
                    # print('epoch: ',epoch,'train  -  ','iter: ',iters,'top1: ',np.array(log_dict[epoch][metric][:len(iters)]))
